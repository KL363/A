...
class LLM_model:
    ...
    def call_llm_stream(self, query, list):
        ...
        prompt = f"""你是一位知识助手，你需要模仿人类的对话风格，将你的回答拆分成3到5个自然段落。每个段落结束后，需要段落与段落之间具有语义的连贯性且不能很生硬，必须使用特殊标记 `[NEW_PARAGRAPH]` 作为换段标志。
...
2. 将完整的回答分成3到5段，分好后的段落需要段落与段落之间具有语义的连贯性且不能很生硬，段落之间必须用 `[NEW_PARAGRAPH]` 分隔。
